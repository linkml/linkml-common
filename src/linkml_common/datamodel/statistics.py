...
class Observation(Event):
    ...
    """
    observation_subject: Optional[Union[Entity,Intangible,PhysicalEntity,Event,Investigation,LifeEvent,Observation,ExecutionOfProcedure,PlannedProcess,InvestigativeProcess,ComputationalPlannedProcess,MaterialCollection,MaterialProcessing,SampleProcessing,SampleCollectionProcess,DataGenerationFromSample,Measurement,QualitativeObservation,PhysicalDevice,SampleMaterial,Agent,CreativeWork,Person,Organization,AutomatedAgent,Concept,InformationEntity,StructuredValue,Role,Relationship,EntitySet,TimePointOrTemporalInterval,Service,Variable,Quantity,QuantityRange,PlannedProcessConfiguration,Prediction,PredictionSummaryStatistics,SimplePrediction,AggregatePrediction,Duration,SimpleQuantity,Ratio,TemporalInterval,TimePoint,TemporalRelationship,PersonInRole,OrganizationPersonnelRelationship,OrganizationalRole,Location,PointLocation,Specification,OrgChart,Publication,Dataset,Procedure,InvestigativeProtocol,StudyDesign,QuantityKind,UnitConcept]] = Field(None)
...
class Prediction(Intangible):
    variable_predicted: Optional[Variable] = Field(None)
    predicted_value: Optional[Any] = Field(None)
    expected_value: Optional[Any] = Field(None)
    type: Literal["Prediction"] = Field("Prediction", description="""A type for a thing. The range of this should be a class within the schema. It is intended for schema-based classification. Anything beneath the shoreline of the schema should use `classification`.""")


class SimplePrediction(Prediction):
    outcome: Optional[OutcomeType] = Field(None)
    expected_value: Optional[Union[float, int, str]] = Field(None)
    variable_predicted: Optional[Variable] = Field(None)
    predicted_value: Optional[Any] = Field(None)
    type: Literal["SimplePrediction"] = Field("SimplePrediction", description="""A type for a thing. The range of this should be a class within the schema. It is intended for schema-based classification. Anything beneath the shoreline of the schema should use `classification`.""")
...
class OutcomeStatistics(ConfiguredBaseModel):
    ...
    """
    true_positive_count: Optional[int] = Field(None, description="""Total number of true positives (TP)""")
    false_positive_count: Optional[int] = Field(None, description="""Total number of false positives (FP)""")
    true_negative_count: Optional[int] = Field(None, description="""Total number of true negatives (TN)""")
    false_negative_count: Optional[int] = Field(None, description="""Total number of false negatives (FN)""")
    recall: Optional[float] = Field(None, description="""Recall is calculated as TP / (TP + FN)""")
    precision: Optional[float] = Field(None, description="""Precision is calculated as TP / (TP + FP)""")
    f1_score: Optional[float] = Field(None, description="""F1 Score is the harmonic mean of precision and recall""")
    accuracy: Optional[float] = Field(None, description="""Accuracy is calculated as (TP + TN) / (TP + FP + TN + FN)""")


class AggregatePrediction(OutcomeStatistics, Prediction):
    true_positive_count: Optional[int] = Field(None, description="""Total number of true positives (TP)""")
    false_positive_count: Optional[int] = Field(None, description="""Total number of false positives (FP)""")
    true_negative_count: Optional[int] = Field(None, description="""Total number of true negatives (TN)""")
    false_negative_count: Optional[int] = Field(None, description="""Total number of false negatives (FN)""")
    ...
    predicted_value: Optional[Any] = Field(None)
    expected_value: Optional[Any] = Field(None)
    type: Literal["AggregatePrediction"] = Field("AggregatePrediction", description="""A type for a thing. The range of this should be a class within the schema. It is intended for schema-based classification. Anything beneath the shoreline of the schema should use `classification`.""")


class PredictionSummaryStatistics(OutcomeStatistics, Intangible):
    """
    A class for collecting totals of various outcomes.
    """
    predictions: Optional[List[Union[Prediction,SimplePrediction,AggregatePrediction]]] = Field(default_factory=list)
    uses_dataset: Optional[str] = Field(None)
    true_positive_count: Optional[int] = Field(None, description="""Total number of true positives (TP)""")
    false_positive_count: Optional[int] = Field(None, description="""Total number of false positives (FP)""")
    true_negative_count: Optional[int] = Field(None, description="""Total number of true negatives (TN)""")
    false_negative_count: Optional[int] = Field(None, description="""Total number of false negatives (FN)""")
    ...
    f1_score: Optional[float] = Field(None, description="""F1 Score is the harmonic mean of precision and recall""")
    accuracy: Optional[float] = Field(None, description="""Accuracy is calculated as (TP + TN) / (TP + FP + TN + FN)""")
    type: Literal["PredictionSummaryStatistics"] = Field("PredictionSummaryStatistics", description="""A type for a thing. The range of this should be a class within the schema. It is intended for schema-based classification. Anything beneath the shoreline of the schema should use `classification`.""")
...
SimplePrediction.update_forward_refs()
OutcomeStatistics.update_forward_refs()
AggregatePrediction.update_forward_refs()
PredictionSummaryStatistics.update_forward_refs()
